...
combine_params <- function(sptl, kmax, lw, cnp, cnpdiet, metpar, artr){
  params <- dplyr::full_join(select(sptl, Family, Species, species), kmax) %>% 
    ...
    mutate(v_m = sst, linf_m = sizemax) %>% select(-sst, -sizemax, -diet_cat) %>% 
...
run_fishflux <- function(tfish_unique, params){
  ...
  cnpflux <- parallel::mclapply(1:nrow(data), function(x){
    ...
    dt <- data[x,] 
    par <- dt %>% select(-species, - Family, - size_cm, -Species) %>% as.list()
...
tfs <- group_by(tfish, studyName, region, locality, sites, area, transect_id, lon, lat) %>% 
  dplyr::summarise(
    ...
    Fp_s = sum(Fp),
    Ic_s = sum(Ic),
    Ic_h_s = sum(Ic[herb == "yes"], na.rm = TRUE),
    Ic_p_s = sum(Ic[herb == "no"], na.rm = TRUE),
    In_s = sum(In),
    Ip_s = sum(Ip),
    Gc_s = sum(Gc),
    Gn_s = sum(Gn),
  ...
  dplyr::summarise(
    ...
    Ic_h_p = sum(Ic[herb == "yes"], na.rm = TRUE)/unique(Ic_h_s),
    Ic_p_p = sum(Ic[herb == "no"], na.rm = TRUE)/unique(Ic_p_s),
...
fit_contribution_Fp <- function(contributions){
  
  ## compile model once 
  subsp <- filter(contributions, species == "Variola_louti")
  subsp$bioregion <- as.factor(as.character(subsp$bioregion))
  fit <- brm("Fp_p ~ (0 + bioregion) + (0 + 1|locality)", chains = 1, family = "beta", data = subsp)
  
  spp <- unique(contributions$species)
  
  # run same model on all species
  list <- parallel::mclapply(spp[1:2], function(x){
    print(x)
    subsp <- filter(contributions, species == x)
    subsp$bioregion <- as.factor(as.character(subsp$bioregion))
    fit_new <- update(fit, newdata = subsp, recompile = FALSE, chains = 2, control = list(adapt_delta = 0.95))
    return(fit_new)
  }, mc.cores = 7)
...
# l2 <- parallel::mclapply(list, function(x){
#   np <- nuts_params(x)
