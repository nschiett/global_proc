...
*We have now substantially changed the first paragraph of the introduction to quickly place the reader in the context of coral reef functioning.*
...
Further, following the recommendation of another reviewer, we now removed this part of analysis entirely. Functional vulnerability is now discussed by combining our results with the known effects of human stressors on fish community structure.*
...
*We now provide more information in the results section.*
...
*This paragraph is now removed entirely because as we removed this part of the analysis as mentioned above.*
...
*This paper aims to demonstrate existing trade-offs among functions and what drives them. In order to have applicable management recommendations, we first need to find out how much of a certain function is 'good'. We are currently still lacking understanding of what a healthy reef really looks like in terms of process-based functions. We added the following sentences: "It is necessary to gauge the state of reef ecosystems based on multiple, complementary, process-based functions, yet our comprehension of
process-based functioning of coral reefs or what a “functional” coral reef really means is still in its infancy.
...
*The model mentioned here is talking about the bioenergetic model provided in the R package 'fishflux' that estimates fluxes of carbon, nitrogen, and phosphorus. We now reworded this section to clarify. We also added mentions of body size and sea surface temperature in the first section of the methods.*
...
*We used the scores from Graham et al. to create a new model and used the model to predict the scores for species that are not provided in that paper. Graham et al. only used three families, so we are aware that this is a big extrapolation. Following reviewer's recommendation, we now removed this analysis from the paper.*
...
*Thank you for the suggestion. This is the only comment that we decided not to include. The paper by Froese and Binohlan is over twenty years old and the data they used does not include reef fishes. Moreover, asymptotic length can vary significantly across locations depending on temperature and other environmental variables. We therefore opted for a method proposed and applied by Renato Morais and colleagues. They standardize asymptotic lenth to maximum length to avoid underestimating growth. In this method, the parameter k, is adapted to kmax in order to represent a realistic growth curve. Doing this may underestimate growth of juvenile fishes, while the potential overestimation of adult fishes would be very minimal. As visual census data often does not accurately record juveniles and we removed small fishes from this specific analysis, we argue that our method minimizes potential bias considering the data available. The risk of using a general conversion factor would be too big as we would estimate a growth equal to zero for a lot of individuals and potentially dramatically underestimate biomass production.*

Line 11: what is this standardized coefficient? Is there a reference?
*We added the definition and reference. "For k, we used a standardized coefficient that describes the potential growth trajectory of an individual if linf were to be equal to its maximum length (i.e., kmax; Morais & Bellwood, 2018)."*
...
*We changed it to "We selected the species of our species list"*

Line 15: I am a bit lost here, what is the “length-frequency method” is this also from Morais?
*The length-frequency method is another, arguably unreliable way to estimate growth trajectories, that is used for some of the data provided on fishbase. We opted to only use growth curve parameters that were estimated from otolith data. We clarified this by saying: "We selected the species of our species list and only included the k estimates coming from otolith studies."*
...
*As mentioned above, we now added text on temperature at the start of the methods section.*
...
*We now clarified this by mentioning otolith data in the previous paragraph.*
...
*Since initial submission, this data was published under the form of a data paper. We now added this reference.*
...
*We added more details on the model.*
...
*"After combining the two data sources, we obtained 496 estimates of kmax for 181 species across varying temperatures."*
...
*It means that most of the variation (independent of size and sst) is explained by phylogeny. This value is thus considered quite high. We now specify this.*
...
*We added a paragraph on the phylogenetic trees.*
...
*Essentially by taking a weighted average of nutrient contents across trophic groups where the weights represent the probability of being in a diet group.*
...
*Thank you. We have now rewritten the manuscript substantially. We focused on imroving clarity, we added a results section, and provided much more details in the methods.*
...
*Thank you. We aimed to clarify all topical sentences.*
...
*We rewrote the introduction and avoided using many contradicting words. We believe the introduction is now more focused.*
...
*We removed the word 'integrity' throughout the manuscript. We also avoided using informal language and removed the given examples.*

21- “integrity” repeated here again. Still unsure what the integrity of an ecosystem means.
*Agreed, it is a rather confusing and vague term. We removed it.*
...
*This section has been changed entirely.*
...
*The second paragraph of the introduction now focuses on fishes.*

33- “integrity” repeated again. Do you mean function?
*No more integrity in this manuscript.*
...
*We removed this sentence and now say: "Drastic declines in habitat quality and fish biomass have evoked serious concerns about the persistence of coral reef functioning."*
...
*As mentioned above, we rewrote the introduction entirely, and hope you will find it has improved in clarity.*
...
*We understand the confusion, and now removed the part of the figure shhowing the contribution of fish families as it was not a main result and distracts the reader from the main point which is indeed that on a local scale a few key species provide high functioning while the identity of these key species vary across sites.*
...
*We now have a seperate results paragraph accompagning figure 4. We hope this clarifies these findings.*
...
*The topical sentence of this paragraph is now: "Our results also reveal that functions consistently hinge on a few dominant species, but the identities of local, dominant species vary strongly across sites."*

108 I’m not sure what a “severe change” is, especially in a broad term like community structure.
*We modified the sentence as followed: "Fishing and climate-induced coral loss have caused declines in reef fish biomass and shifts in community structure."*

112 “effect” used twice in one sentence.
*Changed to: "When accounting for the effect of biomass, these community shifts can..."*

114 is this what Fig 2 shows? Do you mean Fig 4? Unclear
*We removed the mention of the figure in this discussion paragraph as we now have a seperate results section.*
...
*We removed this paragraph as suggested below.*
...
*We removed this paragraph altogether.*
...
*We now edited this first figure to show the actual function estimates per location, as well as a measure of multifunctionality.*
...
*This is now figure 3. We modified the x-axis label to "standardized effect size" and the percentages to lower and upper. We provided extra information in the legend. An individual fish can both be small and mature, so both variables are quite different. Immaturity is a measure that represents the position in the growth curve. We now also added a definition in the legend.
...
*As suggested, we removed this figure and the paragraph surrounding it. Instead, we now provide a conceptual figure that supports this part of the discussion.*
...
*We added more information on the priors everywhere.*
...
*We now added much more information and introductory sentences in the supplementary methods.*
...
*Thank you, we are delighted that you enjoyed reading our paper. We clarified the text as specified below.*
...
*We substantially modified the introduction and added a paragraph on fish-mediated functions specifically. We agree that quantifying all functions proposed by Brandl et al. would have been even better. However, we did not have access to data to do so. For example, CaCO3 production by corals requires on detailed data on coral cover including the community composition and the sizes of corals. We did not have any data on benthic cover for this specific project. We thus only analyzed the functions that are related to fishes based on the data available and the confidence in methodology.*
...
*We now say: "Our results underline the need for a nuanced, local, and context-dependent approach to coral reef conservation that considers multiple functions beyond the effect of standing stock biomass."*
...
*Agreed. However, we now removed this sentence following the suggestion of reviewer 1. MAYBE I SHOULD INCLUDE THIS??*
...
*Done. We now state this explicitely.*

Line 41-42. How exactly is this hindering effective management of reefs? You leave the reader guessing here – poor style.
*We now say: "This sparsity of quantification of functions constitutes a severe limitation to efficient management of coral reef ecosystem functioning." I AM NOT SURE THAT THIS SUFFICES. ANY SUGGESTIONS?*
...
*We simplified this section and now say: ... so a shift to herbivore dominance and the subsequent decline of community-level P excretion may change the balance of nutrient cycling on coral reefs, potentially favoring algal growth in comparison with corals.*
...
*We removed this entire paragraph.*
...
*We added extra details in the legend.*
...
*The color palette was created based on the natural colors of a coral reef fish species: Callanthias australis. We regret that the reviewer found the colors off-putting. We tested the color combination by simulating color-blindness using the website https://coolors.co/ and the red green combination seem to be distinguishable due to differences in clarity. Moreover, as functions are kept in the same order throughout the paper, the results could be interpreted even if there were no colors at all. Therefore, we decided to keep the colors for now, but if this is a big issue for the reviewer or the editor agrees with the reviewer, we will change the color palette.*

Figure 2 seems unnecessarily large – this could be a single column figure.
*Agreed. We decreased the size.*

Figure 4 – Suggest proportion values radiating out run to 1 so piscivory falls within the axis bounds.
*This is a great point, thanks for noticing. However, following a recommendation of reviewer 2, we now actually removed this figure.*
...
*We standardized sizes and fonts as much as possible.*
...
*We used an existing underwater visual census database that has been previously published from. We collected extra data ourselves to parametrize bioenergetic models. We stated this more clearly now.*
