...
# **Title:** Global drivers and vulnerabilities of coral reef fish functions
...
**Abstract:**
Preserving the functioning of coral reefs is a critical challenge of the 21st century.
...
An understanding of such trade-offs is required to make informed management decisions, but simultaneously quantifying multiple ecosystem functions is challenging.
Therefore, trade-offs between functions, their drivers, and their vulnerability are still poorly understood in many ecosystems [@Brandl2019front].
...
However, our capacity to evaluate reef functioning relies on static proxies of functions, such as live coral cover, standing stock biomass of reef fishes, or diversity of qualitative traits [e.g. @Cinner2020].
Conversely, we know comparatively little about actual functions - fluxes of elements and energy - and their drivers (but see [@Allgeier2016]), which constitutes a severe limitation to management [@Brandl2019front].
...
We apply these bioenergetic models to 9,118 reef fish communities in 585 sites worldwide (Table S1) to:
...
Biomass is the most commonly employed indicator of coral reef functioning [@Cinner2020; @Brandl2019front], and we indeed observed a strong relationship between fish biomass and all five functions (Fig. S1a-e, Fig. S2).
However, our analyses demonstrate that functions vary remarkably after accounting for biomass - functions of communities with similar biomass may differ with two orders of magnitude or more (i.e. biomass production, Fig. S1a-f).
Thus, using biomass as a proxy for functioning can mask differences in critical community-level functions.
...
While many reefs may stand out as hotspots for one function, none can maximize functioning as a holistic, multifaceted concept (Fig. 1).
...
For example, phosphorus excretion and piscivory are higher in communities with large-bodied fishes that occupy high trophic levels (Fig. S1g; See @Allgeier2014).
...
Secondly, alongside features of community structure, functions may also be influenced by specific high-performing taxa.
Certain species tend to contribute more or less to a particular function, relative to their contribution to biomass (Fig. 3a; Fig. S3,S4).
Further, abundant species impact rates of functioning at the community level [@Topor2019].
We show that functions consistently hinge on a few dominant species (Fig. 3b). Specifically, more than 50% of a given function, on average, is upheld by only 12% of the species present within a local community.
However, the identity of these species varies dramatically between reefs (Fig. 3c).
...
Our results reveal that while functional dominance is indeed prevalent, the identity of local, dominant species vary strongly across different locations.
...
On the other hand, declines in coral cover related to climate change are often associated with a shift toward herbivores, which may help avoid algal domination [@Graham2006].
However, herbivores have a minor contribution to phosphorus excretion [@Allgeier2014; @Schiettekatte2020], so a shift to herbivore dominance in fish communities and the subsequent decline of community-level phosphorus excretion may change the balance of nutrient cycling on reefs, potentially favoring algal growth [@Burkepile2013].
Thus, considering multiple functions paints a more nuanced picture of how human-induced shifts in reef fish community structure impact coral reef ecosystems.
...
Conserving biomass, diversity, and ecosystem functions are important objectives of most current conservation initiatives [@Cinner2020].
Even though safeguarding fish biomass enhances functioning, the trade-offs between key functions reveal a critical challenge for coral reef conservation, where actions to enhance one function may negatively impact another.
...
Finally, while there is a general consensus on the role of diversity in enhancing functioning [@Zavaleta2010], we highlight the importance of community structure and the identity of dominant species at the local scale.
While, maintaining the diversity of fishes is critical, at local scales, species richness only has a minor impact on individual functions. Rather, dissimilarity between local communities may be critical in maintaining functioning across the seascape since no species consistently provides high contributions for all functions or across its range [@Zavaleta2010].    


Overall, through coral reef fishes, we demonstrate that the variability in processes that govern the cycling of elements presents an unrecognized challenge for protecting ecosystem functioning. Management strategies that call for the enhancement of coral reef functioning via an economic mindset, where higher functioning is better, are not feasible. Instead, conserving coral reef ecosystem functioning will require a more nuanced approach that considers processes that vary beyond the effect of standing stock biomass and are subject to variable, local trade-offs, drivers, and anthropogenic threats.
