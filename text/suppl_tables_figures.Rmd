...
```{r, echo = FALSE}
  ...
  table <- rbind(
    ...
    tidy(list[[2]], par_type = "non-varying") %>%
    mutate_if(is.numeric, round, 4) %>%
    mutate(term = c("intercept",
                    "sst",
                    "log(biomass)",
    ...
    mutate(response = "log(P excretion)"),
    tidy(list[[3]], par_type = "non-varying") %>%
    mutate_if(is.numeric, round, 4) %>%
    mutate(term = c("intercept",
                    "sst",
                    "log(biomass)",
    ...
    mutate(response = "log(Production)"),
    tidy(list[[4]], par_type = "non-varying") %>%
    mutate_if(is.numeric, round, 4) %>%
    mutate(term = c("intercept",
                    "sst",
                    "log(biomass)",
    ...
    mutate(response = "log(Herbivory)"),
    tidy(list[[5]], par_type = "non-varying") %>%
    mutate_if(is.numeric, round, 4) %>%
    mutate(term = c("intercept",
                    "sst",
                    "log(biomass)",
    ...
    mutate(response = "log(Piscivory)")) %>%
    select(response, term, estimate, std.error, lower, upper)
...
![Extended Data Figure 5: Comparison herbivory and piscivory rates when using alternative diet classifications](../output/plots/Supplemental_herbivory_piscivory_alternative_classification.png)
