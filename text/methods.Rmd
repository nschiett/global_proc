...
Finally, we estimated multifunction, i.e. one measure that combines all five functions by taking the geometric average of the five standardized functions. We used the geometric mean because functions are dependent on each other and may vary considerably.
...
\noindent Finally, to investigate the effect of the community structure while still accounting for the effects of standing biomass and sea surface temperature, we fitted a mixed effect multivariate model similar to the model specified above, but adding all community structure variables:
...
All Bayesian models were fitted using the R package *brms* (ref) which uses Stan, a C++ package for performing full Bayesian inference. The posterior distributions of model parameters were estimated using Markov chain Monte Carlo (MCMC) methods by using four chains of 2,000 samples, including 1,000 samples as a warm‐up, meaning that a total of 4,000 samples were used to estimate posterior distributions. The convergence and fit of the models were verified by examining the Rhat, parameter trace plots, and posterior prediction plots. Posterior predictive checks for are provided in Supporting Information xxxx.
...
Finally, we quantified the frequency of dominance per species, i.e. the number of transects in which a species is dominant for a given function divided by the total number of transects in which that species is observed. A species is considered dominant for a certain function in a given transect if their contribution is higher than 1/R, i.e. they contribute more than the situation in which each species contributes equally to a certain function.

## 6. Vulnerability to fishing and climate change
First, we estimated or extracted two measures of vulnerability per species: vulnerability to climate change and vulnerability to fishing pressure [@Graham2011]. For species' vulnerability to climate change, we solely focus on their vulnerability to the loss of live corals. Vulnerability to climate change induced coral loss is related to diet specialization, habitat specialization for live coral, and body size [@Graham2011]. Specifically, we used a score for climate change vulnerability for 134 species found in Graham et al. (2011) [@Graham2011] and fitted a Bayesian mixed effect predictive model that relates these vulnerability scores with the log-transformed maximum size of fish (extracted from Fishbase @Froese2018), the dependence on coral for food (3 categories: not dependent, facultative corallivore, and obligate corallivore), and dependence on coral for habitat (2 categories: dependent vs. not dependent) [@Coker2014; @Cole2008]. We also included a random effect for family:
$$ y_i \sim normal(\mu_i, \sigma)$$
$$
\mu_i = (\beta0 +  \delta_{family}) + \beta1 log(sizemax) + \beta2 HD + \beta3 FD  ,
$$
with $y_i$ the vulnerability score of species i, $\mu_i$ the average predicted vulnerability score, sizemax the maximum length of the species, HD de dependence for habitat, and FD the depedendence for food.
To verify the fit of the model we inspected the posterior predictive plot, which indicated a good fit. Further, the model had a Bayesian R2 of 0.97. We then used this model to extrapolate the vulnerability measure to all 1110 species in our dataset. We provide the model parameters in supplementary table xx. \par For species' vulnerability to fishing, we extracted the index from Cheung et al. (2005) [@Cheung2005].
Next, we calculated vulnerability scores per function on the community level by averaging the species-level scores weighted by the contributions to function of species.
Finally, we fitted a mixed model to predict the average vulnerability to climate change and fishing for each function.
