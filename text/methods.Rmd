...
In the underwater visual transect database, xx transects did not contain herbivores and xx transects did not contain piscivores, yielding false zeros for herbivory and piscivory, respectively. We considered these zeros as missing values because the absence of the observation of a herbivore or piscivore does not prove the actual absence of herbivores and piscivores. In fact, it is highly unlikely that a coral reef fish community contains no herbivores and piscivores at all. To avoid removing all transects with missing values for herbivory or piscivory (n = xx) from our database, we imputed the missing values by fitting two Bayesian regression models using the observed herbivory and piscivory rates and then imputing the missing values by taking the means of posterior predictions. In other words, we used the median predictor values corresponding to missing responses to impute herbivory and piscivory rates. As predictor variables, we used the community structure variables described above and sea surface temperature, all standardized. Additionally, we included random effects of sites and localities on the intercepts.
Both models thus had the following model structure:
$$ y_i \sim normal(\mu_i, \sigma) $$
$$\begin{array}{c}
\mu_{i} = (\beta0 + \delta_{loc} +  \delta_{site}) + \beta_1 x_{log(biomass),i} + \beta_2 x_{sst,i}  + \beta_3 x_{richness,i}  + \beta_4 x_{size_m,i}  + \beta_5 x_{size_{2.5\%},i} +  \beta_6 x_{size_{97.5\%},i} + \\
\beta_7 x_{troph_m,i}  + \beta_8 x_{troph_{2.5\%},i} +  \beta_9 x_{troph_{97.5\%},i} + \beta_{10} x_{imm_m,i}  + \beta_{11} x_{imm_{2.5\%},i} +  \beta_{12} x_{imm_{97.5\%},i} \end{array}$$,

where $i$ represents the transects, $\beta_0$ is the intercept, $\beta_1$ to $\beta_{12}$ are the fixed effect parameters, $\delta_{loc}$ is the random effect of the locality, $\delta_{site}$ is the random effect of the site, biomass represents the total standing stock biomass of the community, sst the sea surface temperature, richness is the species richness, size is the total length, troph is the trophic level, imm is the immaturity, and $m$, $2.5\%$, and $97.5\%$ respectively represent the 50\%, 2.5\%, and 97.5\% quantiles across the fish community.
For these models, we used uninformative priors for the fixed effect parameters ($\beta_{1-12}$) and weakly informative priors for the standard deviations of residuals and random effects (ADD THE PRIOR).
To insure that the performed Imputationdidn't change main results of the downstream regression models, we repeated all below-mentioned models on a subset of the transect data, excluding the original missing values. Results of models are shown in the supplementary information xxxxxxx.

### 4.2 Regressions with biomass and SST, and estimates of correlation

The standing stock biomass of communities is inevitably positively related to all functions because of the additive nature of the quantification. The relationships between functions and biomass are likely to be allometric following metabolic theory. Therefore, we log-transformed each function and biomass in all regression analyses. Furthermore, because of the known relationship between temperature and parameters related to growth and respiration [@Morais2018; refMETABOLIC], functions are hypothesised to be affected by  temperature. To model the effect of biomass and SST, while simultaneously investigating the correlations between the the five functions (independent of biomass and SST), we performed a multivariate Bayesian mixed effect regression model:


$$\begin{bmatrix}log(Y_{ExN,i}) \\ log(Y_{ExP,i}) \\ log(Y_{Prod,i}) \\ log(Y_{Herb,i}) \\ log(Y_{Pisc,i}) \end{bmatrix} \sim MVNormal\left(\begin{bmatrix} \mu_{ExN,i} \\ \mu_{ExP,i} \\ \mu_{Prod,i} \\ \mu_{Herb,i} \\ \mu_{Pisc,i}\end{bmatrix}, S \right),$$
...
0 & \sigma_{ExP} & 0 & 0 & 0 \\with the exclusion of
0 & 0 & \sigma_{Prod,i} & 0 & 0\\
0 & 0 & 0 & \sigma_{Herb} & 0\\
0 & 0 & 0 & 0 & \sigma_{Pisc} \end{bmatrix}$$
...
where i is the transect, Y_{ExN,i} is N excretion, Y_{ExP,i} is P excretion, Y_{Prod,i} is biomass production, Y_{Herb,i} is herbivory, Y_{ExN,i} is piscivory, \sigma represents the residual error of each function (ExN, ExP, Prod, Herb, and Pisc), R is the correlation matrix of the residuals. Locality- and site-level effects are also structured including covariation among functions. There are thus three correlation matrices in total, or in other words the model will estimate the correlation between functions (independent of biomass and SST) on three levels: locality-level, site-level, and transect-level.


\noindent We used non-centered parametrisation for site and location effects and all standard deviations had the following prior: $\sigma \sim student(3, 0, 2.5)$. We used a lkj_corr prior for each of the three correlation matrices ($R \sim lkj\_corr(1)$) (ADD REF). We used weakly-informative normal priors for the model slopes ($\beta1 \sim normal(1,1)$, $\beta2 \sim normal(0,1)$).


### 4.3 Intercepts-only models
Additionally, we performed a multivariate intercept-only model to estimate the functions per locality, and a univariate intercept-only model to estimate the multifunction per locality. The model structures were similar as the above-mentioned models, the only difference being the exclusion of the predictor variables (sst and biomass).
Further, we fitted a similar univariate model to predict the multifunction per locality:
$$\mu_{\textrm{multi,i}} = \beta0_{multi} +  \delta_{multi, loc} +  \delta_{multi, site} $$



### 4.4 Effect of community structure on ecosystem functions
...
For all models we inspected the MCMC chains and the Rhat for convergence and peformed posterior predictive checks (Appendix xxxx).
We used Bayesian R2 to estimate the amount of explained variation of each model.
